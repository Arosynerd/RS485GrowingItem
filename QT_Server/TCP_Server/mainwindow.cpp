                                          ...
                                          ui(new Ui::MainWindow)
{
    ...
    serilaPort = new QSerialPort(this);//USB通信端口控制
...
void MainWindow::socket_espRead_Data()
{
    ...
    QString data = QString(buffer); // 接收数据格式:年份月份日期小时分钟秒（14位）温度湿度光强度（各占6位，共18位）
...
void MainWindow::timerEvent(QTimerEvent *event)
{
    if (event->timerId() != timer_id)
        return;
    count++;
    ui->label_11->setNum(count);

   QString serialNamePort;
   foreach (const QSerialPortInfo &info ,QSerialPortInfo::availablePorts()) {
       serialNamePort = info.portName();
       //输出调试信息
       qDebug() << "serialNamePort:" << serialNamePort;
       if(serialNamePort.contains("COM14")){

           qDebug() << "找到USB端口";
           if(USB_Init()){
               ui->label_11->setStyleSheet("QLabel { color : green; }");
               ui->label_11->setText("USB连接");
               //关闭定时器
           ...
           else qDebug() << "失败打开USB端口";
       }
   }
   if(ui->label_11->text() != "USB连接")
   qDebug() << "无usb连接";
}
void MainWindow::serilaPortReadyRead_Slot()
{
    QByteArray buffer = serilaPort->readAll();
    QString data = QString(buffer); // 接收数据格式:年份月份日期小时分钟秒（14位）温度湿度光强度（各占6位，共18位）
    ...
    // 更新UI
    QString tem = data.mid(14, 6);
    QString light = data.mid(26, 6);
    QString tem1 = data.mid(32, 6);
    QString light1 = data.mid(44, 6);
...
bool MainWindow::USB_Init(){
    ...
    if(serilaPort->open(QIODevice::ReadWrite) == true){
        qDebug() << "成功打开串口：" << serilaPort->portName();
        return true;
    }else{
        qDebug() << "失败打开串口：" << serilaPort->portName();
        return false;
    }
